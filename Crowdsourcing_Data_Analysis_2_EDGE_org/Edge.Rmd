...
Another `r percent(length(count_numberOfChars$n[count_numberOfChars$n == 2])/nrow(edgeDS))` of comments has a character 
lenght of 2.
...
p1 <- ggparcoord(data = count_numberOfChars_small, columns = c(1, 2), scale = "globalminmax") + 
  ...
  labs(y = "Number of occurances", x = "", title = "TOP 20 connections - from # of comments to lenght of comments") +
  scale_fill_ptol() + 
  theme_minimal() + 
...
p2 <- ggparcoord(data = count_numberOfChars_small, columns = c(2, 1), scale = "globalminmax") + 
  ...
  labs(y = "Number of characters", x = "", title = "TOP 20 connections - from lenght of comments to # of comments") +
  scale_fill_ptol() + 
  theme_minimal() + 
  theme(axis.title.x=element_blank(), 
        axis.text.x=element_blank(), 
...
Depending on how one calculates the (avarage) female participitation, it has still a growing trend. 
...
```{r include=FALSE, message=F, warning=F}
# Now, remove datasets to free RAM
rm(PeoplePerThreadID,edgeDS_conversations_q1,women_part_by_year,edgeDS_conversations_q1, q1,q2, r1, r2)
```
