...
- You have to execute this Rmd file in its directory/folder - not in the root! of repository.
...
First, I start with (non-)academicians and hence need right data.
...
What is number of contributions by both genders per year.
...
a lot of people were searching for job at home and had a lot of time to invest in EDGE conversations. 

Both, the unique number of contributors (left plot) and total number of contributions in debates is growing in both genders. 

```{r, fig.height=8, fig.width=15, message=F, warning=F }
r1 <- ggplot(edgeDS_conversations_q1, aes(x= Year)) + 
  geom_point(aes(y = Total_UniqueMaleContributors, colour = "Male Contributions")) + 
  geom_line(aes(y = Total_UniqueMaleContributors, colour = "Male Contributions")) +
  stat_smooth(aes(y = Total_UniqueMaleContributors, colour = "Male Contributions"), 
              method = "lm", col = "#1695A3") + 
  geom_point(aes(y = Total_UniqueFemaleContributors, colour = "Female Contributions")) + 
  geom_line(aes(y = Total_UniqueFemaleContributors, colour = "Female Contributions")) +
  stat_smooth(aes(y = Total_UniqueFemaleContributors, colour = "Female Contributions"), 
              method = "lm", col = "#450003") + 
  ylab("# of Unique people contributing by Gender") + labs(color = "Gender") + 
  ggtitle("Unique Contributors") +
...
r2 <- ggplot(edgeDS_conversations_q1, aes(x= Year)) + 
  ...
  theme_minimal() + scale_fill_ptol() +
  scale_x_continuous(breaks = pretty(edgeDS_conversations_q1$Year, n = 19)) + 
  scale_y_continuous(breaks = seq(0, 1300, by = 150), limits = c(0, 1300))
...
We can now look on how has participitation of women increased over the years - considering that there might be 
differences between those in Academia with PhD. or where the author and commenter are the same person.  

The plot below shows e.g. that there are no women who are academicians and dont have their PhD. Every women is either academician with PhD. (makes sence) or is a non-academician having PhD or not. 
...
ggplot(women_part_by_year, aes(x=Year, y = Female_Contributions)) + 
  geom_point() + facet_grid(Academic ~ HavePhD) + 
  theme_minimal() + scale_fill_ptol() + ggtitle("Female contributions and impact of being academician with/out PhD.")

ggplot(women_part_by_year, aes(x=Year, y = Female_Contributions)) + 
  geom_point() + facet_grid(AuthorAndCommenter ~ TwoAuthors) + 
  theme_minimal() + scale_fill_ptol() + ggtitle("Female contributions to the articles where the author and commenter is same person", subtitle = "and where comments can be written by two people")

ggplot(women_part_by_year, aes(x=Year, y = Female_Contributions)) + 
  geom_point() + facet_grid(Live ~ . ) + 
  theme_minimal() + scale_fill_ptol() + ggtitle("Female contributions to the Transcribed or Written Text Pieces")
```

## Female participation per Year

First, again prepare the data for later use.
...
Independanly of how one calculates the (avarage) female participitation, the growing is still trend. 
...
Idea: If an author is a woman, then there is potentially a higher engagement with other women. Yet correlation doesnt confirm this. 

```{r}
cor_data <- edgeDS %>% 
  filter(Role == 1, Gender == "Female", Type == 2) %>% 
  select(-Link, -Id_num, -ThreadId, -Year, -Title,-Role,-Type, -Order, -Text,-Limited_Information,-Gender, -starts_with("dummy"))
...
```{r include=FALSE, message=F, warning=F}
# Now, remove datasets to free RAM
rm(PeoplePerThreadID,edgeDS_conversations_q1,women_part_by_year, all_char_names,
...
**My Definition:** I define "higher status" as being solely dependent on the person's background, workplace (i.e. where he comes from), his/her job (and if academicians, then also the department, discipline and academic rank, citations), whether (s)he has a PhD and from which institution. 
