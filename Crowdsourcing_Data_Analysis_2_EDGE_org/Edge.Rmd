...
arrange(aggr(mis, col=mdc(1:2), numbers=TRUE, sortVars=TRUE, labels=names(mis), 
        cex.axis = .4, gap=3, prop = F, plot = F)$missings, desc(Count))[1:50, ]
```
...
```{r}
e1071::skewness(women_part_by_year$Female_Contributions[women_part_by_year$Live == "Transcribed TP"])
```

`Skewness` function confirms that contributions of women to the transcribed text pieces is visibly skewed to the left (being '+') - i.e. the [curve is right-leaning](https://en.wikipedia.org/wiki/Skewness#Introduction).
