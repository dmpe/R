...
The plot below shows e.g. that there are no women who are academicians and dont have their PhD. Every women is either academician with PhD. or is a non-academician with either PhD or without one. 
...
On the other hand, the bottom-left distribution is more normally distrubuted than others having the lowest skewness of just 1.4. 
...
```{r, message=F, warning=F}
skew_fem_tran <- moments::skewness(women_part_by_year$Female_Contributions[women_part_by_year$Live == "Transcribed TP"])
agostino.test(women_part_by_year$Female_Contributions[women_part_by_year$Live == "Transcribed TP"])

skew_fem_writ <- moments::skewness(women_part_by_year$Female_Contributions[women_part_by_year$Live == "Written TP"])
agostino.test(women_part_by_year$Female_Contributions[women_part_by_year$Live == "Written TP"])



skew_m_tran <- moments::skewness(men_part_by_year$Male_Contributions[men_part_by_year$Live == "Transcribed TP"])

skew_m_writ <- moments::skewness(men_part_by_year$Male_Contributions[men_part_by_year$Live == "Written TP"])
...
edgeDS_verbosity_disc_diff <- edgeDS_verbosity %>% 
  ...
  summarize(min_Number.Characters = min(Number.Characters), 
            ...
            mean_Number.Characters = round(mean(Number.Characters),1)
            ) %>% 
  mutate(difference_max_min = max_Number.Characters-min_Number.Characters)
```
...
ggplotly(
  ggplot(edgeDS_verbosity, aes(Gender, Number.Characters)) + geom_boxplot() +
    scale_y_continuous(breaks = seq(0,2000, by = 200), limits = c(0,2000)) +
    theme_minimal() + scale_fill_ptol()
) %>% layout(xaxis = list(ticktext = c("Male", "Female", "NA")))
```
...
Filter Phd + high ranking + work -> vs. all others
