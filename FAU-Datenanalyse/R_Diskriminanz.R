...
bewerber=read.table("Bewerber.txt",header=T,dec=",",sep=";")
attach(bewerber)
...
mu=(mu1+mu2+mu3)/3
B=(mu1-mu)%*%t(mu1-mu)+(mu2-mu)%*%t(mu2-mu)+(mu3-mu)%*%t(mu3-mu)
(EW=eigen(solve(Sp)%*%B))
(DK=EW$vectors)
(Wilks=prod(1/(1+EW$values)))
(chi=-(nrow(bewerber)-(2+3)/2-1)*log(Wilks))
qchisq(0.95,2*(3-1))
(d1=t(mu1)%*%solve(Sp)%*%c(2.7,530)-1/2*t(mu1)%*%solve(Sp)%*%mu1+log(1/3))
(d2=t(mu2)%*%solve(Sp)%*%c(2.7,530)-1/2*t(mu2)%*%solve(Sp)%*%mu2+log(1/3))
(d3=t(mu3)%*%solve(Sp)%*%c(2.7,530)-1/2*t(mu3)%*%solve(Sp)%*%mu3+log(1/3))
library(MASS)
...
Ergeb3 = lda(Gruppe ~ ., data=bewerber, prior=c(1/3,1/3,1/3), subset=lern)
