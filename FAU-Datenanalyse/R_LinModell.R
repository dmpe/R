...
############################################################################
### Aufgabe 7
un=read.table("Unternehmen.csv",sep=";",dec=",",header=T,row.names=1) 
str(un)
colnames(un)=c("CEOGehalt", "Sektor" ,"EBIT","Umsatz","ROE","EKQuote")
which(is.na(un)==TRUE)
str(un)					
summary(un)
(fit=summary(lm(CEOGehalt~ROE+Umsatz+Sektor+EBIT+EKQuote, data=un)))
...
(fitoAeD=summary(lm(CEOGehalt~0+ROE+Umsatz+SektorIuII.Dummy+EBIT+EKQuote, data=un)))
(fitoAbD=summary(lm(CEOGehalt~0+ROE+Umsatz+Sektor+EBIT+EKQuote, data=un)))
un$FKQuote=1-un$EKQuote
(fit=summary(lm(CEOGehalt~ROE+Umsatz+Sektor+EBIT+EKQuote+FKQuote, data=un)))
