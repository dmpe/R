...
Produkte=read.table("Produkte.csv",sep=";",dec=",",header=T)
...
TukeyHSD(aov(Bewertungen~Preissegment,data=Produkte),conf.level=0.99)
...
(X=model.matrix(~Preissegment*Verwendung,data=Produkte,contrasts.arg=list(Preissegment="contr.sum",Verwendung="contr.sum")))
t(X)%*%X
solve(t(X)%*%X)
...
interaction.plot(Produkte$Preissegment, Produkte$Verwendung, Produkte$Bewertung)
interaction.plot(Produkte$Verwendung, Produkte$Preissegment, Produkte$Bewertung)
