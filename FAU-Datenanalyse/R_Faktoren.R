...
(ML=factanal(covmat=as.matrix(stkor),factors=2,rotation ="none"))
...
factanal(covmat=as.matrix(stkor), factors=2, rotation="varimax")
Ladung.ML%*%ML.var$rotmat
(HK.var=varimax(Ladung.HK))
Ladung.ML.var=ML.var$loadings
Ladung.HK.var=HK.var$loadings
par(mfrow=c(2,2))
plot(Ladung.ML[,1],Ladung.ML[,2],type="n",ylim=c(-1,1),xlim=c(-1,1),xaxs="i",yaxs="i")
text(Ladung.ML[,1],Ladung.ML[,2],labels=1:5,col=2)
abline(v=0,lty=2)
abline(h=0,lty=2)
plot(Ladung.HK[,1],Ladung.HK[,2],type="n",ylim=c(-1,1),xlim=c(-1,1),xaxs="i",yaxs="i")
text(Ladung.HK[,1],Ladung.HK[,2],labels=1:5,col=4)
abline(v=0,lty=2)
abline(h=0,lty=2)
plot(Ladung.ML.var[,1],Ladung.ML.var[,2],type="n",ylim=c(-1,1),xlim=c(-1,1),xaxs="i",yaxs="i")
text(Ladung.ML.var[,1],Ladung.ML.var[,2],labels=1:5,col=4)
abline(v=0,lty=2)
abline(h=0,lty=2)
plot(Ladung.HK.var[,1],Ladung.HK.var[,2],type="n",ylim=c(-1,1),xlim=c(-1,1),xaxs="i",yaxs="i")
text(Ladung.HK.var[,1],Ladung.HK.var[,2],labels=1:5,col=3)
abline(v=0,lty=2)
abline(h=0,lty=2)
