...
dist(users)
users_m=t(matrix(rep(colMeans(users),nrow(users)),nrow=ncol(users),byrow=F))
...
users_norm=sweep(users_norm,2,STAT=sqrt(((nrow(users)-1)/nrow(users))*diag(cov(users))),FUN="/")
...
points(KM$centers[,1],KM$centers[,3],col=1:3,pch=2,cex=2)
